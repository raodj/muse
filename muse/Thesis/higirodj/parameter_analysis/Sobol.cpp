...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        cout << "Specify: #Dimensions #NumSets #Skip\n";
        return 1;
    }
    // Extract command-line arguments
    const int dims = atoi(argv[1]);
    const int rows = atoi(argv[2]);
    const int skip = atoi(argv[3]);
    // Generate dim * rows of random numbers
    float* const rnds = i4_sobol_generate(dims, rows, skip);
    // Print dims * rows of random numbers.
    for (int row = 0; (row < rows); row++) {
        for (int dim = 0, idx = row * dims; (dim < dims); dim++, idx++) {
            cout << rnds[idx] << " ";
        }
        cout << std::endl;
    }
