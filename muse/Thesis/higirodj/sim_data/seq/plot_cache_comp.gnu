...
outFile = config . "_cache_info.pdf"
...
set ylabel "Average cache misses (%)" font " Bold, 8" offset 2,0
...
getData(queue) = sprintf("< cut -d',' -f3-5,60-77 %s | grep '%s'", getCsvFile(queue), searchStr)
...
plot for [i=1:4] getData(word(queueList, i)) using 3:($15*100/$6):($16*100/$7) with filledcurves lc rgb word(ColorList, word(queueColor, i) + 0) notitle,\
     for [i=1:4] getData(word(queueList, i)) using 3:($14*100/$4):($15*100/$6):($16*100/$7) with yerrorbars lw 1 ps 0.2 pt 7 lc rgb word(ColorList, word(queueColor, i) + 0) notitle,\
     for [i=1:4] getData(word(queueList, i)) using 3:3 with lines lw 2 lc rgb word(ColorList, word(queueColor, i) + 0) title word(queueList, i)
