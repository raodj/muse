...
different parameters~\cite{higiro2017multi}.
...
simulations using PHOLD~\cite{higiro2017multi}.}\label{fig:gsa-anal-3tvs2t}}
\end{figure}

Interestingly, other model parameters such as \textbf{rows}, \textbf{cols}, \textbf{self-events}, \textbf{simEndTime}, and \textbf{granularity} have no influence on relative performance of \textbf{2tLadderQ} vs \textbf{3tHeap}. The parameter with most influence is \textbf{eventsPerLP} with a score of 0.774. This parameter determines the total number of concurrent events which influences bucket sizes and number of rungs in \textbf{2tLadderQ} as well as the third tier size in \textbf{3tHeap}. The parameter $\lambda$ for exponential distribution has a marginal influence because it influences number of concurrent events as discussed in Section~\ref{sec:pholdModel} and shown in Figure~\ref{fig:phold}(c)~\cite{higiro2017multi}.
...
scheduler queues for consistent comparison and analysis~\cite{higiro2017multi}. This consideration was not applied to the \textbf{PCS} simulation to remain consistent with the simulation properties as presented in \cite{carothers-94}.
...
vs. \textbf{3tHeap})~\cite{ higiro2017multi}.}\label{fig:par-gsa-anal-3tvs2t}}
\end{figure}
...
behaviors, etc. In order to pursue such analysis we have used 6 different configurations for the simulations. The \textbf{PHOLD} configurations are called \textbf{ph3}, \textbf{ph4}, and \textbf{ph5} and the \textbf{PCS} configurations are called \textbf{pcs6}, \textbf{pcs7}, and \textbf{pcs8}. The fixed characteristics for the 6 configurations is summarized in Table~\ref{tab:configs}. We use larger simulation end times for parallel simulation to obtain sufficiently long runtimes using 32 cores. The value of influential parameters, namely: \textbf{eventsPerLP}, \textbf{\%selfEvents}, and \textbf{$\lambda$} for the \textbf{PHOLD} configuration and \textbf{portables} for \textbf{PCS} is varied for comparing different settings, similar to the approach used by other investigators~\cite{tang-05,franceschini-15, higiro2017multi}.

\begin{table}[htbp]\centering
\textbf{\caption{Configurations of PHOLD and PCS used for further analysis}\label{tab:configs}}
\begin{tabular}{llll}
\toprule
...
\end{table}
\newpage
\section{ Sequential Simulations}\label{sec:seq}
...
parameters~\cite{ higiro2017multi}}}\label{fig:seq-time}
\end{figure}
...
\end{minipage}
\textbf{\caption{Comparison of peak memory usage~\cite{ higiro2017multi}}\label{fig:seq-mem}}
\end{figure}


\subsection{PCS sequential simulation results}\label{sec:pcs_seq}
...
\includegraphics[width=\linewidth, height = 6.5cm]{images/pcs6_runtime_full}
\centerline{(a) \textbf{pcs6}}
\end{minipage}
\begin{minipage}{0.75\linewidth}
\includegraphics[width=\linewidth, height = 6.5cm]{images/pcs7_runtime_full_log}
\centerline{(b) \textbf{pcs7}}
\end{minipage}

\begin{minipage}{0.75\linewidth}
\includegraphics[width=\linewidth, height = 6.5cm]{images/pcs8_runtime_full_log}
...
\centering{
\begin{minipage}{0.75\linewidth}
\includegraphics[width=\linewidth, height = 6.5cm]{images/pcs6_runtime_partial2}
\centerline{(a) \textbf{pcs6}}
\end{minipage}
\begin{minipage}{0.75\linewidth}
\includegraphics[width=\linewidth, height = 6.5cm]{images/pcs7_runtime_partial2}
\centerline{(b) \textbf{pcs7}}
\end{minipage}

\begin{minipage}{0.75\linewidth}
\includegraphics[width=\linewidth, height = 6.5cm]{images/pcs8_runtime_partial2}
\centerline{(c) \textbf{pcs8}}
\end{minipage}
 }
...
\section{Parallel simulation assessments}\label{sec:par}
...
The charts in Figure~\ref{fig:par-time-ph3}, Figure~\ref{fig:par-time-ph4}, and Figure~\ref{fig:par-time-ph5} show key simulation statistics for low value of \textbf{eventsPerLP} = 2 and $\lambda$=1 for
...
\begin{figure}
  ...
  \textbf{\caption{Statistics from PH3 configuration of PHOLD parallel simulation with \textbf{eventsPerLP}=2, $\lambda=1$, \textbf{\%selfEvents}=25\%~\cite{ higiro2017multi}}\label{fig:par-time-ph3}}
\end{figure}
...
\begin{figure}
  ...
  \end{minipage}
  \textbf{\caption{Statistics from PH4 configuration of PHOLD parallel simulation with \textbf{eventsPerLP}=2, $\lambda=1$, \textbf{\%selfEvents}=25\%~\cite{ higiro2017multi}}\label{fig:par-time-ph4}}
\end{figure}
...
\centerline{(c) \textbf{ph5} time \& rollbacks}    
\end{minipage}  
\textbf{\caption{Statistics from PHOLD parallel simulation with \textbf{eventsPerLP}=10, $\lambda=10$, \textbf{\%selfEvents}=25\%~\cite{ higiro2017multi}}\label{fig:par-time-2}}
\end{figure}
...
\end{minipage}
\textbf{\caption{\textbf{ph5} Statistics (best case for \textbf{3tHeap})~\cite{ higiro2017multi}}\label{fig:par-time-3}}
\end{figure}
...
\subsection{Best case for 3tHeap}
