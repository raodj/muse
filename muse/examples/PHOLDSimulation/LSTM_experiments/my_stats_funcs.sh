...
function summarize_runtime_stats() {
    ...
    for outFile in `ls -1 ${prefix}*`; do
        # If the first character on grep result is not a space then we
        # need adjust the column number.
        local fileStatCol=${statCol}
	statList=( `grep "$srchStr" ${outFile} | awk '{$1=$1;print}' | cut -d' ' -f${fileStatCol} | tr -d ',' | sort -n` )
	...
	if [ ${#statList[*]} -gt 0 ]; then
		...
		modelStats[${fileNum}]=${statList[$maxIdx]}
            elif [ "${aggrType}" == "avg" ]; then
                local meanCI=( `calcMeanAndCI ${statList[@]}` )
                modelStats[${fileNum}]=${meanCI[0]}
	    else
...
function summarize_elapsed_time_stats() {
    >&2 echo "Sorry this function is not tested!"
    exit
    # Set variable names to make function more readable
    local prefix=$1
    local entries=$2
	...
	local statList=( `grep "$srchStr" ${outFile} | tr -s ' ' | cut -d' ' -f${statCol}` )
...
# Summarizes cache misses/instruction from perf statistics.
#  $1 = Prefix of the files in pwd to be summarized
#  $2 = number of entries expected in each file -- ignored
#  $3 = x-axis value -- ignored
#  $4 = summary data file -- ignored
#  $5 = Search string to identify line of statistic -- ignored
#  $6 = Column in search line containing numeric statistic -- ignored
#  $7 = "one" or "sum" to select one or sum of values in this stats -- ignored
function summarize_cache_miss_per_instr {
    # Set variable names to make function more readable
    local prefix=$1
    # Extract total number of cache misses from the file.
    local cacheMisses=( `grep " cache-misses " ${outFile}* | tr ":" " " | tr -s ' ' | cut -d' ' -f2 | tr -d ','` )
    local instructions=( `grep " instructions " ${outFile}* | tr ":" " " | tr -s ' ' | cut -d' ' -f2 | tr -d ','` )
    if [ "${instructions[0]}" == "instructions" ]; then
        >&2 echo "Error extracting instructions from ${outFile}"
        exit
    fi
    # >&2 echo "${outFile} cacheMisses = ${cacheMisses[@]}"
    # >&2 echo "${outFile} instructions = ${instructions[@]}"
    # Compute the fraction of cache misses
    local missPerInstr=()
    local numEntries=$(( ${#cacheMisses[@]} - 1 ))
    for i in `seq 0 ${numEntries}`
    do
        missPerInstr[${i}]=`echo "${cacheMisses[$i]} / ${instructions[$i]}" | bc -l`
    done
    missPerInstr=( `echo ${missPerInstr[@]} | tr " " "\n" | sort -n` )
    # >&2 echo "${outFile} missPerInstr = ${missPerInstr[@]}"
    # Now we have the synthesized statistics from all the files. Print
    # summary values for plotting averages and confidence intervals.
    echo "missPerInstr ${#missPerInstr[*]} `calcMeanAndCI ${missPerInstr[*]}` `calcBoxVals ${missPerInstr[*]}`"
}

# Computes normalized runtime @ 2.4 GHz for each run using the formula:
#      NormTime = cycles / %cpu / 2.4 / 10^9
#
#  $1 = Prefix of the files in pwd to be summarized
#  $2 = number of entries expected in each file -- ignored
#  $3 = x-axis value -- ignored
...
#  $6 = Column in search line containing numeric statistic -- ignored
#  $7 = "one" or "sum" to select one or sum of values in this stats -- ignored
function summarize_norm_runtime {
    # Set variable names to make function more readable
    local prefix="$1"
    # Extract nessary data from output files while accounting for
    # presence/absence of a leading blank space.
    local cycles=( `grep " GHz" ${outFile}* | tr ":" " " | tr -s ' ' | cut -d' ' -f2 | tr -d ','` )
    local ghz=( `grep " GHz" ${outFile}* | tr ":" " " | tr -s ' ' | cut -d' ' -f5` )
    # Extract the CPU utilization for this run.
    local cpuUtil=( `grep " CPUs utilized" ${outFile}* | tr ":" " " | tr -s ' ' | cut -d' ' -f6` )
    # Compute the normalized runtime for each entry
    local normRuntime=()
    local numEntries=$(( ${#cpuUtil[@]} - 1 ))
    for i in `seq 0 ${numEntries}`
    do
        normRuntime[${i}]=`echo "${cycles[$i]} / ${cpuUtil[$i]} / 2.4 / 10^9" | bc -l`
        # >&2 echo "${cycles[$i]} / ${cpuUtil[$i]} / 2.4GHz = ${normRuntime[$i]}"
    done
    normRuntime=( `echo ${normRuntime[@]} | tr " " "\n" | sort -n` )
    # >&2 echo "${outFile} normRuntime = ${normRuntime[@]}"
    # Now we have the synthesized statistics from all the files. Print
    # summary values for plotting averages and confidence intervals.
    echo "normRuntime ${#normRuntime[*]} `calcMeanAndCI ${normRuntime[*]}` `calcBoxVals ${normRuntime[*]}`"
}
