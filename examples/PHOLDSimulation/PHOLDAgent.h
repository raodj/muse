...
public:
    ...
    */
    enum DelayType { UNIFORM, POISSON, EXPONENTIAL, REVERSE_POISSON,
                     REVERSE_EXPONENTIAL, LOCAL_REMOTE, INVALID_DELAY };
    ...
    */
    ...
    /** Method to set the range of local agent IDs.

        This method is used by PHOLDSimulation::createAgents to set
        the range of local agents (i.e., agents partitioned to the
        same thread/process).

        \note The partitioning is assumed to be done in contiguous
        blocks.

        \param[in] min The minimum ID of the local agent.

        \param[in] max The maximum ID of the local agent.

        \param[in] remoteEvtFrac The fraction of remote events to be
        generated.  This value is used only if the receiver
        distribution is set to LOCAL_REMOTE.
    */
    void setLocalAgentRange(const muse::AgentID min, const muse::AgentID max,
                            const double remoteEvtFrac = 0.0);
...
protected:
    ...
    */
    ...
    /** Simulate some processing of data/bytes associated with an
        event.

        This method adds the character values of each data value in
        the event and ensures it is consistent.  This simulates some
        minor event processing by inspecting values in the data.

        \param[in] event The event to be inspected/processed.
    */
    void simEventProcessing(const muse::Event* event) const;

    /** Helper method to get ID of a local or remote agent.

        This method is called from getRecvrAgentID when the
        receiverDistType is LOCAL_REMOTE. This method uses the
        remoteEvents fraction to determine if a local or remote agent
        ID is to be generated.  Next, it uniformly selects a
        local/remote agent ID and returns the agent ID.

        \return The ID of the agent to which the next event should be
        sent.
    */
    muse::AgentID getLocalRemoteID();
...
private:
    ...
    */
    ...
    /** Additional bytes to be added to an event to facilitate
        ...
        This value is set when the agent is instantiated and is never
        changed during the life time of this agent.
    */
    int extraEventSize;
    ...
    */
    ...
    /** The fraction of remote events to generate when the
        receiverDistType is LOCAL_REMOTE.

        This value must be set via the \c --remote-events command-line
        argument.  This value is used when the user chooses a
        LOCAL_REMOTE distribution where a fraction of events are
        sent to local agents (on the same thread/process as indicated
        by localAgentRange) while rest of the events are sent to
        remote agents.
    */
    double remoteEvents;

    /** This pair maintains the minimum and maximum IDs of the local
        agents assigned to this thread/process.

        The partition of agents in PHOLDSimulation::createAgents is
        done in contiguous blocks, where each process/thread gets a
        contiguous range of agents.  This pair maintains the minimum
        (in localAgentRange.first) and maximum
        (localAgentRange.second) of the range of agents assigned to
        this thread/process.
    */
    std::pair<int, int> localAgentRange;
};
