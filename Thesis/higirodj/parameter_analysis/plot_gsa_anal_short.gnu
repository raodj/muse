...
# NOTE: This script is the same as plot_gsa_anal.gnu except it plots a
# shorter version of the chart with horizontal x-axis labels etc.
# This script was created to make charts shorter to fit PADS/ACM
# format better.
...
genData(x) = system(sprintf("./extract_gsa_for_plot_shortLabel.sh %s %s", suffix, csvLog))
...
do for[idx=0:MaxVars] {
    ...
    if (fVal < 0.5) {
        set label labStr center at idx, fVal + ci + 0.1 font " Bold, 8"
    } else {
        set label labStr center at idx, fVal + ci + 0.1 font " Bold, 8"
    }
}
